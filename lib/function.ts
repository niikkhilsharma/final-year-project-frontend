...
export async function uploadToCloudinary(file: File) {
	...
	console.log(`https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/${data.publicId}`)
	return `https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/${data.publicId}`
}

export async function downloadImage(imgUrl: string, imageName: string = 'Model') {
